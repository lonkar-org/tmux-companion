...
| `note [TEXT]` | Leave a one-line note on a pane, as tmux's pane title, which `panes` shows beside the program and the pane border draws when `pane-border-status` is on. No text prints the note that is there; `--clear` takes it off; `--pane ID` picks the pane, defaulting to the one the key was pressed in |
