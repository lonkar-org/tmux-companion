...
| 8 | the config loader and its tables | done (loader, errors, `config` subcommand, `[general]`, `[dirs]`, `[git]`, `[network]`, `[battery]`); `[glyphs]`, `[git] parts`, `[status.right]` and `[[layout]]` still todo |
