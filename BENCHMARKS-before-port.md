# Benchmarks: the port itself

These are the before and after numbers for the port, taken in August 2026 while
the Rust daemon was replacing the zsh scripts and the Go `yrl`. They answer one
question -- did collapsing five `#()` calls into one actually cost less -- and
they answer it by measuring the socket rather than the session.

[BENCHMARKS.md](./BENCHMARKS.md) is the live document: what the tool costs
today, measured from inside a real tmux with real key presses. Start there. This
file is kept because it is where the design reasoning was worked out and the
figures it quotes are the ones every commit message from that period cites.
...
```sh
python3 scripts/bench-cpu.py target/release/tmux-companion "after" --tmux
python3 scripts/bench-cpu.py /path/to/old/binary  "before" --tmux
```

`bench.sh`, the wall-clock harness these figures also quote, is gone. It
needed `gdate`, it timed a client rather than a session, and what replaced it
is `scripts/bench.sh`, which drives real key presses inside a real tmux.
...
Wall-clock round trips including process spawn, from the deleted `bench.sh`. Kept for
